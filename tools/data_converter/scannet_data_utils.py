...
def get_scene_list():
    ...
    scene_list = []
    for s in split_list:
        with open(f'/fast_data/jhuangce/mmdetection3d/data/scannet/meta_data/scannet_rpn_{s}.txt', 'r') as f:
            scene_list += [x.strip() for x in f.readlines()]
    return scene_list

class ScanNetRPNData(object):
    """ScanRPNNet data.
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_filename = osp.join(self.root_dir, 'scannet_projected_point_cloud',
                                    f'{sample_idx}.npy')
            ...
            info['pts_path'] = osp.join('scannet_projected_point_cloud',
                                        f'{sample_idx}.npy')
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if annotations['gt_num'] != 0:
                    ...
                    annotations['name'] = np.array([
                        0
                        for _ in range(annotations['gt_num'])
                    ])
                    # default names are given to aligned bbox for compatibility
                    # we also save unaligned bbox info with marked names
                    annotations['location'] = aligned_box[:, :3]
